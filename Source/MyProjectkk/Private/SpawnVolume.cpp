...
void ASpawnVolume::SpawnOurPawn_Implementation(UClass* pawn ,const FVector& Location)
{
	if (pawn) {
		...
		if (World) {
			ACritter* CritterSpawned = World->SpawnActor<ACritter>(pawn, Location, FRotator(0.0f), SpawnParams );
		}
	}
}
